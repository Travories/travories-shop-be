...
- `.env` — the single root env file shared by both apps; never commit, print, or copy secret values out of it. Edit `.env.example` instead when documenting a new variable. Do not add per-app `.env` / `.env.local` files.
